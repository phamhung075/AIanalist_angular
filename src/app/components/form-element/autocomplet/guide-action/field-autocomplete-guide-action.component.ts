...
import { GuideAction } from '../../../models/GuideAction';
...
import { GuideActionService } from '../../../services/guide-action/guide-action.service';

@Component({
	selector: 'FIELD_autocomplete--guide-action',
	standalone: true,
	imports: [
		CommonModule,
		MatIconModule,
	...
	templateUrl: './field-autocomplete-guide-action.component.html',
	styleUrl: '../../../scss/autocomplete.scss',
	encapsulation: ViewEncapsulation.Emulated,
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FieldAutocompleteGuideActionComponent extends FormControlAutocompleteBaseDirective implements OnDestroy, OnChanges {
	...
	loading = false;
	filteredOptions!: Observable<GuideAction[]>;
	...
	private _filter(filterText: string): Observable<GuideAction[]> {
		...
		return from(this.guideActionService.fetchPageByAcademieObjectId_Autocomplete(filterText, this.academieAudite_objectId, 1, 10)).pipe(
			map((response) => {
				this.loading = false;
	...
	displayFn(guideAction?: GuideAction): string {
		return guideAction && guideAction?.titre
			? guideAction?.titre
			: '';
	}

	onOptionSelected(event: MatAutocompleteSelectedEvent): void {
		const value: GuideAction = event.option?.value;
		if (!value) {
			return;
		}
