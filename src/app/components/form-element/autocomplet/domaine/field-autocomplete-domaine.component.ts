...
@Component({
	selector: 'FIELD_autocomplete--domaine',
	standalone: true,
	imports: [
		CommonModule,
		MatIconModule,
	...
	templateUrl: './field-autocomplete-domaine.component.html',
	styleUrl: '../../../scss/autocomplete.scss',
	encapsulation: ViewEncapsulation.Emulated,
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FieldAutocompleteDomaineComponent extends FormControlAutocompleteBaseDirective {
	...
	private _filter(filterText: string): Observable<Domaine[]> {
		...
		return from(this.domaineService.fetchPage(filterText, 1, 10)).pipe(
			map((response) => {
				this.loading = false;
	...
	displayFn(domaine?: Domaine): string {
		return domaine && domaine.titre
			? domaine.titre
			: '';
	}

	onOptionSelected(event: MatAutocompleteSelectedEvent): void {
		const value: Domaine = event.option.value;
		if (!value) {
			return;
		}
