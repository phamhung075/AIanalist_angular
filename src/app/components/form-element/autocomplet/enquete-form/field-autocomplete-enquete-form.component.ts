...
import { EnqueteForm } from '../../../models/EnqueteRecueil';
...
import { EnqueteFormService } from '../../../../public-api';

@Component({
	selector: 'FIELD_autocomplete--enquete-form',
	standalone: true,
	imports: [
		CommonModule,
		MatIconModule,
	...
	templateUrl: './field-autocomplete-enquete-form.component.html',
	styleUrl: '../../../scss/autocomplete.scss',
	encapsulation: ViewEncapsulation.Emulated,
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FieldAutocompleteEnqueteFormComponent extends FormControlAutocompleteBaseDirective implements OnDestroy, OnChanges {
	...
	loading = false;
	filteredOptions!: Observable<EnqueteForm[]>;
	...
	private _filter(filterText: string): Observable<EnqueteForm[]> {
		...
		return from(this.enqueteFormService.fetchPageByAcademieObjectId_Autocomplete(filterText, this.academieAudite_objectId, 1, 10)).pipe(
			map((response) => {
				this.loading = false;
	...
	displayFn(enqueteForm?: EnqueteForm): string {
		return enqueteForm && enqueteForm?.titre
			? enqueteForm?.titre
			: '';
	}

	onOptionSelected(event: MatAutocompleteSelectedEvent): void {
		const value: EnqueteForm = event.option?.value;
		if (!value) {
			return;
		}
