...
import { FichePoste } from '../../../models/FichePoste';
...
import { FichePosteService } from '../../../services/fiche-poste/fiche-poste.service';

@Component({
	selector: 'FIELD_autocomplete--fiche-poste',
	standalone: true,
	imports: [
		CommonModule,
		MatIconModule,
	...
	templateUrl: './field-autocomplete-fiche-poste.component.html',
	styleUrl: '../../../scss/autocomplete.scss',
	encapsulation: ViewEncapsulation.Emulated,
	changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FieldAutocompleteFichePosteComponent extends FormControlAutocompleteBaseDirective implements OnDestroy, OnChanges {
	...
	loading = false;
	filteredOptions!: Observable<FichePoste[]>;
	...
	private _filter(filterText: string): Observable<FichePoste[]> {
		...
		return from(this.fichePosteService.fetchPageByAcademieObjectId_Autocomplete(filterText, this.academieAudite_objectId, 1, 10)).pipe(
			map((response) => {
				this.loading = false;
	...
	displayFn(fichePoste?: Partial<FichePoste>): string {
		return fichePoste && fichePoste?.titre
			? fichePoste?.titre
			: '';
	}

	onOptionSelected(event: MatAutocompleteSelectedEvent): void {
		const value: FichePoste = event.option?.value;
		if (!value) {
			return;
		}
